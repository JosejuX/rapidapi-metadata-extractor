...
def test_unsafe_csp_scores_lower_than_strict_csp():
    ...
    _, weak_score, weak_grades = extract_security_headers(weak_headers)
    _, strong_score, strong_grades = extract_security_headers(strong_headers)
...
def test_missing_header_scores_lower_than_weak_header():
    ...
    _, empty_score, empty_grades = extract_security_headers(empty_headers)
    _, weak_score, weak_grades = extract_security_headers(weak_headers)
...
def test_hsts_short_max_age_is_weak_long_max_age_with_subdomains_is_strong():
    ...
    _, _, short_grades = extract_security_headers(short)
    _, _, long_grades = extract_security_headers(long_with_subdomains)
...
def test_x_content_type_options_only_nosniff_is_strong():
    ...
    _, _, nosniff_grades = extract_security_headers(headers_nosniff)
    _, _, garbage_grades = extract_security_headers(headers_garbage)
...
def test_referrer_policy_unsafe_url_is_weak_no_referrer_is_strong():
    ...
    _, _, weak_grades = extract_security_headers(weak)
    _, _, strong_grades = extract_security_headers(strong)
...
def test_all_headers_missing_scores_zero():
    _, score, grades = extract_security_headers({})
    assert score == 0.0
...
def test_best_possible_headers_score_highest_but_permissions_policy_caps_below_strong():
    ...
    _, score, grades = extract_security_headers(headers)
